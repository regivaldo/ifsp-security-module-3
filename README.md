...
Como um usuário não autenticado, acesse as páginas de acesso restrito da aplicação (páginas em "/cadastro", "/administradores" e "/usuarios"). O acesso deve ser negado para ambas as páginas (usuários não autenticados não devem acessar páginas restritas a usuários autenticados);  
Efetue login na aplicação como um usuário administrador e acesse as páginas em "/cadastro", "/administradores" e "/usuarios". O acesso deve ser permitido para as páginas em "/cadastro" e "/administradores". O acesso deve ser negado para a página em "/usuarios" (usuários administradores não devem acessar a página de acesso restrito aos usuários comuns);
Ainda como usuário administrador, acesse a página em "/cadastro" e cadastre um usuário do tipo "usuário administrador" e um usuário do tipo "usuário comum";
Encerre a sessão do usuário administrador;
Efetue login na aplicação utilizando a conta do usuário comum criada no item 3 e acesse as páginas em "/cadastro", "/administradores" e "/usuarios". O acesso deve ser permitido para a página em "/usuarios" e negado para as páginas em "/cadastro" e "/administradores" (usuários comuns não devem acessar as páginas de acesso restrito aos usuários administradores);
Encerre a sessão do usuário comum;
Mostre o "ponto único" utilizado para implementar os mecanismos de segurança da aplicação;
Mostre o mecanismo de segurança para prevenir ataques de injeção;
Mostre o mecanismo de segurança para prevenir ataques de CSRF;
...
Mostre o mecanismo de segurança para prevenir ataques nas sessões dos usuários;
Mostre o mecanismo de segurança para o tratamento de erros da aplicação, incluindo a abordagem para a não exibição de mensagens de erro detalhadas para o usuário;
Mostre o mecanismo de segurança para gerar os hashes das senhas dos usuários, incluindo os hashes gerados para os usuários cadastrados na aplicação (mostre os hashes armazenados no banco de dados).
